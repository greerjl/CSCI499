			       ...
			       <div class="form_main">
				       ...
				       <div id="" style="overflow-y: scroll; height:100px">
				         <table>
				             <?php
				             	$gid = $_SESSION["gid"];
				               $sql = "SELECT name, time FROM event WHERE GID = $gid";
				               $result = mysqli_query($db, $sql);
				             ...
				             <?php
				               ...
				               if($count == 0){
				                 ...
				                 while ($line = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				                 		...
				                 		if(date($formatDate) == date($formatDate, strtotime($date))) {
				                 			echo "\t\t<tr><td><b>$user</b></td><td><b>"."---on---"."</b></td><td><b>".date($formatAll, strtotime($date))."</b></td></tr>";//as bold
				                 		}
				                 		else{
												echo "\t\t<tr><td>$user</td><td>"."---on---"."</td><td>".date($formatAll, strtotime($date))."</td></tr>";
				                 		}
				                 }//while
				               }//else
				             ?>
